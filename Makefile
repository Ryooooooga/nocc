...
%.o: %.c nocc.h file.h map.h path.h util.h vec.h
	${CC} ${CFLAGS} -c -o $@ $<

%-2.ll: %.c nocc.h file.h map.h path.h util.h vec.h nocc
	./nocc $< > $@

%-3.ll: %.c nocc.h file.h map.h path.h util.h vec.h nocc_stage2
	./nocc_stage2 $< > $@
